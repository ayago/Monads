...
public abstract class MaybeMonad<T> extends Monad<MaybeMonad<T>, T> {
  ...
  @Override
  public <U> MaybeMonad<U> bind(Function<T, ? extends Monad<?, U>> f) {
  ...
  static class Just<T> extends MaybeMonad<T> {
  ...
  static class Nothing<T> extends MaybeMonad<T> {

    private static final Nothing NOTHING = new Nothing();

    static <U> Nothing<U> instance() {
      return NOTHING;
    }
